...
public class FrontendProjectService {
    ...
    public FrontendProject getProject(int id, Locale locale) throws Exception {
        Optional<Project> optionalProject = projectRepository.findById(id);
        if(!optionalProject.isPresent())
            return null;

        Project project = optionalProject.get();
        return new FrontendProjectAssembler(project).assemble(locale);
    }

    public List<FrontendProject> getAllProjects(Locale locale) throws Exception {
    ...
    private Project createProject(String title, String locatie, String text, int bannerIndex, int frontIndex, String... imageUrls) throws Exception {
        ...
        Project project = new Project(title, locatie, text, projectImages, bannerIndex, frontIndex);
        if(project.getTitle("") == "City Garden"){
            project.setText_nl("Amstelveen IS HEEL LEUK");
            project.setText_en("Amstelveen IS SO COOL");
            project.setText_zh("SJING SJONG");
            project.setTitel_en("Amstelveen man");
            project.setLocatie_en("Location man");
        }
    ...
    public void createProjects() throws Exception {
        Project amstelveen = createProject("City Garden", "Amstelveen", "", 0, 1,
