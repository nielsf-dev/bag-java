...
var winDow = $(window);
	...
	var OpenDropdown = $('a.open-dropdown'),
	...
	try {
	    var fenway = [52.297356, 4.990951]; //Change a map coordinate here!
	    var markerPosition = [52.297356, 4.990951]; //Change a map marker here!
    	$('#map')
			.gmap3({
				center:fenway,
				zoom: 15,
	...
	LoadButton.live( 'click', function(event) {
		...
		$(storage).load(LoadContainer + " .project-item", function(){
			...
			if ( !( s < elemloadedLength ) ) {
			    $('a.load-post-container').text("meer projecten of specifieke onderwerpen op aanvraag");
			}
